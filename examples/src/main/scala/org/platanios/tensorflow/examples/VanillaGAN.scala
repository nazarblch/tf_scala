...
object VanillaGAN extends App {
  ...
  def gen_rand(n: Int, p: Int): Tensor = {
    Tensor.fromArrayFloat(
      Array.fill(n * p)(2 * r.nextGaussian() - 1).map(_.toFloat)
    ).reshape(Shape(n, p))
  }
  ...
  println(mnist.shape(0) + " " + mnist.shape(1))

  val image_dim = 784 // 28*28 pixels
  val gen_hidden_dim =  256
  val disc_hidden_dim = 100
  val noise_dim = 50 // Noise data points
  ...
  val weights = Map(
    "gen_hidden1" -> tf.variable("gen_hidden1", FLOAT32, Shape(noise_dim, gen_hidden_dim), RandomNormalInitializer(standardDeviation = 1d / noise_dim)),
    "gen_out" -> tf.variable("gen_out", FLOAT32, Shape(gen_hidden_dim, image_dim),  RandomNormalInitializer(standardDeviation = 1d / gen_hidden_dim)),
  ...
  val  biases = Map(
    "gen_hidden1" -> tf.variable("gen_hidden1_bias", FLOAT32, Shape(gen_hidden_dim),  RandomNormalInitializer()),
    "gen_out" -> tf.variable("gen_out_bias", FLOAT32, Shape(image_dim), RandomNormalInitializer()),
    "disc_hidden1" -> tf.variable("disc_hidden1_bias", FLOAT32, Shape(disc_hidden_dim), RandomNormalInitializer()),
    "disc_out" -> tf.variable("disc_out_bias", FLOAT32, Shape(1), RandomNormalInitializer())
  )


  val res: Tensor = 1- Tensor.ones(FLOAT32, Shape(100))
  ...
  def discriminator(x: Output): Output = {
    val hidden_layer = tf.relu(tf.matmul(x, weights("disc_hidden1")) + biases("disc_hidden1"))
    val out_layer = tf.sigmoid(tf.matmul(hidden_layer , weights("disc_out")) + biases("disc_out"))
    out_layer
  }
  ...
  // Build Loss
  val gen_loss = tf.mean(tf.log(Tensor.ones(FLOAT32, Shape(batch_size)).toOutput - discriminator(gen_sample) ))
  val disc_loss = -tf.mean(tf.log(disc_real + 0.00001)) -
    tf.mean(tf.log(Tensor.ones(FLOAT32, Shape(batch_size)).toOutput - discriminator(gen_sample) ))


  // Build Optimizers
  val optimizer_gen = tf.train.Adam(learning_rate )
  val optimizer_disc = tf.train.Adam(learning_rate)
  ...
  val train_gen = optimizer_gen.minimize(-disc_loss, variables=gen_vars, name = "gen_min")
  val train_disc = optimizer_disc.minimize(disc_loss, variables=disc_vars, name = "disc_min")
  ...
  for(i <- 0 until num_steps) {
    ...
    var from = (i * batch_size) % (mnist.shape(0) - batch_size)
    ...
    if(i % 4 == 0) session.run(feeds = feeds, targets = train_disc)
    session.run(feeds = feeds, targets = train_gen)

    val res: Seq[Tensor] = session.run(feeds = feeds, fetches = Seq(gen_loss, disc_loss, disc_fake.toOutput))
